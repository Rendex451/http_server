...
static _Bool _tree_eq(vtype_t tkey, vtype_t tvalue, tree_node *x, tree_node *y);
...
_Bool tree_eq(Tree *x, Tree *y) {
    ...
    return _tree_eq(x->type.key, x->type.value, x->node, y->node);
}

_Bool _tree_eq(vtype_t tkey, vtype_t tvalue, tree_node *x, tree_node *y) {
    if (x == NULL && y == NULL) {
        return 1;
    }
    if (x != NULL && y != NULL) {
        ...
        return fkey && fval && _tree_eq(tkey, tvalue, x->left, y->left) && _tree_eq(tkey, tvalue, x->right, y->right);
    }
    return 0;
}
